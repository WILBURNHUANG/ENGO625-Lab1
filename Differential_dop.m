...
for i = 1:n_epoch_ref
    ...
    if(n_sat_ref(i) >= min_sat)
        while (n_iteration < 10)
            %update
            [X_ref(i,:), ~ , ~  , ~ , ~ , ~ ] = LS_adjustment(XR0_ref,0,XS(index_ref,:), pr_s_ref(index_ref));
...
% 
% sat_index = zeros(n_epoch,5); %for dop_LS
% 
% %Data Matching
% for i = 1:n_epoch
%     for k = 0:11
...
%         while (n_iteration < 10)
%             [XR(i,:),dT(i),EDOP(i),NDOP(i),VDOP(i),HDOP(i),PDOP(i),Cx_hat(i,:),RES{i},sat_index(i,:)] = ...
%                 dop_SD_LS_adjustment(XR0,x_base,XS(index,:),pr_s(index),pr_s_ref(index));
% 
%             delta_x = XR0 - XR(i,:);
% 
%             %exit judgement
...
for i = 1:n_epoch
    ...
    if(n_sat_common(i) >= min_sat)
        while (n_iteration < 10)
            [XR(i,:),EDOP(i),NDOP(i),VDOP(i),HDOP(i),PDOP(i),Cx_hat(i,:),RES{i},sat_index(i,:)] = ...
                dop_DD_LS_adjustment_B(XR0,X_ref(i,:),XS(index,:),pr_s(index),pr_s_ref(index),pivot(i));
...
for i = 1:n_epoch
    for j = 1:5
        sat_index(i,j) = sat_index(i,j) + 12*(i-1);
        CHO{i}(j) = prn_s(sat_index(i,j));
    end
end
...
p26 = p26(p26 ~= 0);t26 = t26(p26 ~= 0);e26 = e26(p26 ~= 0);
p27 = p27(p27 ~= 0);t27 = t27(p27 ~= 0);e27 = e27(p27 ~= 0);
